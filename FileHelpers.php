...
class FileHelpers
{
    ...
    function __construct($taxonomy='files', $subPath='', $global=false) {
        global $currentSystem, $currentCustomer,$currentUser;
        ...
        if ($global){
            ...
            $this->file_path = $currentCustomer->code.'/'.$subPath;
        }
        if (!Storage::disk()->exists($this->file_path.'/')) {
            Storage::disk()->makeDirectory($this->file_path.'/');
        }
        if (!Storage::disk()->exists('tmp5/')) {
            Storage::disk()->makeDirectory('tmp5/');
        }
        if (!empty($currentSystem->settings)){
            if (!empty($currentSystem->settings->options)){
                if (!empty($currentSystem->settings->options->thumb_width)){
                    self::$thumb_size['width'] = (int)$currentSystem->settings->options->thumb_width;
                }
                if (!empty($currentSystem->settings->options->thumb_height)){
                    self::$thumb_size['height'] = (int)$currentSystem->settings->options->thumb_height;
                }
                if (!empty($currentSystem->settings->options->xs_width)){
                    self::$xs_size['width'] = (int)$currentSystem->settings->options->xs_width;
                }
                if (!empty($currentSystem->settings->options->xs_height)){
                    self::$xs_size['height'] = (int)$currentSystem->settings->options->xs_height;
                }
                if (!empty($currentSystem->settings->options->md_width)){
                    self::$md_size['width'] = (int)$currentSystem->settings->options->md_width;
                }
                if (!empty($currentSystem->settings->options->md_height)){
                    self::$md_size['height'] = (int)$currentSystem->settings->options->md_height;
                }
                if (!empty($currentSystem->settings->options->lg_width)){
                    self::$lg_size['width'] = (int)$currentSystem->settings->options->lg_width;
                }
                if (!empty($currentSystem->settings->options->lg_height)){
                    self::$lg_size['height'] = (int)$currentSystem->settings->options->lg_height;
                }
            }
        }
    }
    ...
    {
        global $currentCustomer, $currentUser,$currentPerson;
        if ($this->status){
    ...
    {
        global $currentSystem, $currentCustomer;
        $file_path = $currentCustomer->code.'/'.$currentSystem->system.'/'.$subPath;
        if (Storage::disk()->exists($file_path)) {
            Storage::disk()->deleteDirectory($file_path);
        }
    }
    public static function createFolder($subPath): void
    {
        ...
        $file_path = $currentCustomer->code.'/'.$currentSystem->system.'/'.$subPath;
        if (!Storage::disk()->exists($file_path)) {
            Storage::disk()->makeDirectory($file_path);
        }
    }
    public static function renameFolder($oldName, $newName): void
    {
        global $currentSystem, $currentCustomer;
        $file_path_old = $currentCustomer->code.'/'.$currentSystem->system.'/'.$oldName;
        $file_path_new = $currentCustomer->code.'/'.$currentSystem->system.'/'.$newName;
        Storage::disk()->makeDirectory($file_path_new);
        if (!Storage::disk()->exists($file_path_old)) {
            Storage::disk()->move($file_path_old, $file_path_new);
        }
    }
    ...
    public function imageUploader($file): static
    {
        if (!empty($file)) {
    ...
    public function uploader($file): static
    {
        if (!empty($file)) {
            $this->setFileName($file->getClientOriginalName());
            $this->file_size = $file['size'];
    ...
    public function destroyFileByPath($parts): bool{
        global $currentSystem, $currentCustomer;
        if ($parts){
            if (str_contains($parts['dirname'], $currentCustomer->code)){
    ...
    public static function destroy($file): bool{
        global $currentSystem, $currentCustomer;
        if ($file){
            if (str_contains($file->file_path, $currentCustomer->code)){
    ...
    {
        global $currentSystem, $currentCustomer;
        return Storage::disk()->directories($currentCustomer->code.'/'.$currentSystem->system.'/', $recursive);
    }

    public function getAllFiles($subPath='/'): array
    {
        global $currentSystem, $currentCustomer;
        return Storage::disk()->allFiles($currentCustomer->code.'/'.$currentSystem->system.$subPath);
    }
