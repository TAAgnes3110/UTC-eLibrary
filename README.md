...
- [Backend đã triển khai: Phân loại & Kho](#-backend-đã-triển-khai-phân-loại--kho)
...
### 7. Backend đã triển khai: Phân loại & Kho

Hệ thống đã triển khai đầy đủ backend cho **phân loại sách (DDC)**, **phân loại sách chi tiết** và **kho sách**, gồm API CRUD, nhập/xuất Excel và file mẫu cho người dùng.

#### Phân loại sách (Classifications — DDC)

- **Chuẩn:** Hệ thống Phân loại Thập phân Dewey (DDC), 7 lớp chính dùng tại UTC: `000`, `300`, `500`, `600`, `700`, `800`, `900`.
- **Bảng:** `classifications` (id, code, name, parent_id, params).
- **API (prefix `/api/v1/classifications`):**
  - `GET /` — Danh sách có phân trang, tìm kiếm `?keyword=`
  - `GET /list` — Danh sách tất cả (dropdown)
  - `GET /{id}` — Chi tiết một phân loại
  - `GET /{id}/details` — Danh sách phân loại chi tiết thuộc phân loại đó (dropdown)
  - `POST /` — Tạo mới
  - `PUT /{id}` — Cập nhật
  - `DELETE /{id}` — Xóa
  - `GET /export` — Xuất Excel (query `?ids[]=` tùy chọn)
  - `GET /import-template` — Tải file mẫu nhập
  - `POST /import` — Nhập từ file Excel (xlsx/xls/csv)
- **Seeder:** `ClassificationSeeder` — 7 lớp DDC (000, 300, 500, 600, 700, 800, 900).
- **File mẫu (gửi cho người dùng):** `storage/app/public/Mẫu nhập phân loại sách.xlsx` — Cột: **Mã phân loại**, **Tên phân loại** (dữ liệu từ dòng 3).

#### Phân loại sách chi tiết (Classification details)

- **Bảng:** `classification_details` (id, code, name, classification_id, parent_id, params).
- **API (prefix `/api/v1/classification-details`):**
  - `GET /` — Danh sách có phân trang, tìm kiếm `?keyword=`, lọc `?classification_id=`
  - `GET /{id}` — Chi tiết
  - `POST /` — Tạo mới
  - `PUT /{id}` — Cập nhật
  - `DELETE /{id}` — Xóa
  ...
  - `GET /import-template` — Tải file mẫu nhập
  - `POST /import` — Nhập từ file Excel (xlsx/xls/csv)
- **Seeder:** `ClassificationDetailSeeder` — Các nhánh DDC tiêu biểu (004, 005, 330, 620, 624, 625, 629, 720, 895.922, 910, …).
- **File mẫu (gửi cho người dùng):** `storage/app/public/Mẫu nhập phân loại sách chi tiết.xlsx` — Cột: **Mã phân loại chính**, **Mã phân loại chi tiết**, **Tên phân loại chi tiết** (dữ liệu từ dòng 3). Dòng nào có mã phân loại chính không tồn tại sẽ bị bỏ qua khi import.

#### Kho sách (Warehouses)

- **Bảng:** `warehouses` (id, code, name, parent_id, is_active, params).
- **Seeder:** `WarehouseSeeder` — 3 kho UTC: **Kho Giáo trình (Tầng 1 - Nhà A8)**, **Kho Sách tham khảo & Luận án (Tầng 5–6 - Nhà A8)**, **Kho Tài liệu số**.
- **API:** CRUD, nhập/xuất Excel, tải mẫu (tương tự phân loại). File mẫu: `storage/app/public/Mẫu nhập kho sách.xlsx`.

#### Master data & Sách

- **GET `/api/v1/master-data`** trả về `classifications` và `classification_details` (đã cache) để form sách chọn phân loại/chi tiết.
- Sách (`books`) có `classification_id`, `classification_detail_id`, `warehouse_id`; trường `book_code` dùng lưu call number đầy đủ (VD: `625.7 N527a`).
...
| **Imports/** | Nhập Excel (ClassificationImport, ClassificationDetailImport, WarehouseImport) |
| **Exports/** | Xuất Excel (SimpleTableExport) |
