...
Ảnh tổng quan quan hệ các bảng nghiệp vụ chính (MySQL), phục vụ sắp xếp schema / import vào công cụ vẽ ERD. Một số bảng framework (token, OTP, Spatie permission, cache, queue…) được lược bớt trên sơ đồ để dễ đọc.

| |
|:--:|
| ![Sơ đồ ERD UTC eLibrary](docs/utc-elibrary-erd.png) |
| *UTC eLibrary — ERD (khái niệm); chi tiết cột và FK: `database/migrations/*`* |

**File:** [`docs/utc-elibrary-erd.png`](docs/utc-elibrary-erd.png)
...
- **ERD (ảnh):** [`docs/utc-elibrary-erd.png`](docs/utc-elibrary-erd.png) — cùng nội dung với mục [Sơ đồ CSDL (ERD)](#-sơ-đồ-csdl-erd) ở trên.
