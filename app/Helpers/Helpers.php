...
class Helpers
{
    ...
    public static function generateUsername($name)
    {
        ...
        while (!$ok) {
            if (DB::table($userTable)->where('username', $username)->exists()) {
    ...
    public static function generatePersonUsername($person_id, $prefix): string
    {
        ...
        if (!DB::table($userTable)->where('username', '=', $username)->exists())
            return $username;
        $maxUser = DB::table($userTable)->max('id');
    ...
    public static function getPermissionValues($system): array
    {
        global $role_prefix;
        $groups = config($system . '.permissions');
        $results = [];
        if ($groups) {
            foreach ($groups as $group) {
                if (!empty($group['permissions'])) {
                    foreach ($group['permissions'] as $permission) {
                        $results[] = $role_prefix . $permission['id'];
                    }
                }
            }
        }
    ...
    {
        global $role_prefix, $currentSystem;
        $groups = config($currentSystem->system->value . '.permissions');
        if ($groups) {
            foreach ($groups as &$group) {
                if (!empty($group['permissions'])) {
                    foreach ($group['permissions'] as &$permission) {
                        $permission['id'] = $role_prefix . $permission['id'];
                    }
                }
            }
        }
    ...
    public static function getSystemMenuTypes(): array
    {
        global $currentSystem;
        return config($currentSystem->system->value . '.menu_types');
    }
    public static function getKieuVanBan(): array
    {
        return config('app.kieu_van_ban');
    }
    public static function getSystemBlockTypes(): array
    {
        global $currentSystem;
        return config($currentSystem->system->value . '.block_types');
    }
    public static function getSystemTemplates(): array
    {
        global $currentSystem;
        $config = config($currentSystem->system->value);
        $templates = $config['templates'];
        if (!empty($config[$currentSystem->customer_code])) {
            if (!empty($config[$currentSystem->customer_code]['templates'])) {
                $templates = array_merge($templates, $config[$currentSystem->customer_code]['templates']);
            }
        }
        return $templates;
    }
    public static function getCurrentSystemTemplate()
    {
        global $currentSystem;
        if (!empty($currentSystem->settings)) {
            if (!empty($currentSystem->settings->options)) {
                if (!empty($currentSystem->settings->options->template)) {
                    return $currentSystem->settings->options->template;
                }
            }
        }
        return $currentSystem->system->value . '_default';
    }
    public static function getCurrentSystemDefaultSettings($page = ''): array
    {
        global $currentSystem;
        if (!empty($currentSystem->settings)) {
            if (!empty($currentSystem->settings->pages)) {
                if ($page) {
                    return (array)$currentSystem->settings->pages->$page;
                }
                return (array)$currentSystem->settings->pages;
            }
        }
        return [];
    }
    public static function getSystemTemplateLayouts($template, $page = ''): array
    {
        global $currentSystem;
        //if ($currentSystem->system->value == 'sportal'){
        if ($page) {
            return config('templates.' . $currentSystem->system->value . '.' . $template . '.' . $page);
        } else {
            return config('templates.' . $currentSystem->system->value . '.' . $template);
        }
        /*}else{
            $template_layouts = config( $currentSystem->system->value.'.template_layouts');
            if ($page){
                if (!empty($template_layouts[$template][$page])){
                    return $template_layouts[$template][$page];
                }
                return [];
            }
            return $template_layouts[$template];
        }*/
    }
    public static function getSystemTemplatePageLayouts($template, $page = ''): array
    {
        global $currentSystem;
        if ($page) {
            return config('templates.' . $currentSystem->system->value . '.' . $template . '.' . $page);
    ...
    public static function getSystemTemplateHooksList($template, $page = ''): array
    {
        global $currentSystem;
        $res = [];
        if ($page) {
            $positions =  config('templates.' . $currentSystem->system->value . '.' . $template . '.' . $page);
            if ($positions) {
                foreach ($positions as $position) {

                    if (!empty($position['hooks'])) {
                        $g = ['text' => $position['title'], 'children' => []];
                        foreach ($position['hooks'] as $id => $text) {
                            $g['children'][] = ['id' => $id, 'text' => $text];
                            //$res[] = ['id'=>$id, 'text'=>$text];
                        }
                        $res[] = $g;
                    }
    ...
    public static function getSystemTemplateBlockLayout($block)
    {
        global $currentSystem;
        $template = $currentSystem->system->value . '_default';
        if (!empty($currentSystem->settings)) {
            if (!empty($currentSystem->settings->options)) {
                if (!empty($currentSystem->settings->options->template)) {
                    $template = $currentSystem->settings->options->template;
                }
            }
        }
        if ($block) {
            try {
                $block_templates = config('templates.' . $currentSystem->system->value . '.' . $template . '.blocks.' . $block);
                if ($block_templates) {
    ...
    {
        global $role_prefix;
        return 'role_or_permission:' . \App\Enums\RoleType::SUPER_ADMIN->value . "|" . \App\Enums\RoleType::LIBRARIAN->value;
    }
    public static function ropAdmin(): string
    {
        global $role_prefix;
        return 'role_or_permission:' . \App\Enums\RoleType::SUPER_ADMIN->value . "|" . \App\Enums\RoleType::LIBRARIAN->value . "|{$role_prefix}" . \App\Enums\RoleType::ADMIN->value;
    }
    public static function ropManager(): string
    {
        global $role_prefix;
        return 'role_or_permission:' . \App\Enums\RoleType::SUPER_ADMIN->value . "|" . \App\Enums\RoleType::LIBRARIAN->value . "|{$role_prefix}" . \App\Enums\RoleType::ADMIN->value . "|{$role_prefix}MANAGER_SYSTEM";
    }
    public static function ropKetLuanDanhGia(): string
    {
        global $role_prefix;
        return 'role_or_permission:' . \App\Enums\RoleType::SUPER_ADMIN->value . "|" . \App\Enums\RoleType::LIBRARIAN->value . "|{$role_prefix}" . \App\Enums\RoleType::ADMIN->value . "|{$role_prefix}KET_LUAN_DANH_GIA";
    }
    public static function ropManagerDocument(): string
    {
        global $role_prefix;
        return 'role_or_permission:' . RoleType::SUPER_ADMIN->value . "|" . RoleType::LIBRARIAN->value . "|{$role_prefix}" . RoleType::ADMIN->value . "|{$role_prefix}MANAGER_DOCUMENT";
    }
    public static function ropDocument(): string
    {
        global $role_prefix;
        return 'role_or_permission:' . RoleType::SUPER_ADMIN->value . "|" . RoleType::LIBRARIAN->value . "|{$role_prefix}" . RoleType::ADMIN->value . "|{$role_prefix}MANAGER_DOCUMENT" . "|{$role_prefix}DOCUMENT";
    }
    public static function ropCanteen(): string
    {
        global $role_prefix;
        return 'role_or_permission:' . RoleType::SUPER_ADMIN->value . "|" . RoleType::LIBRARIAN->value . "|{$role_prefix}" . RoleType::ADMIN->value . "|{$role_prefix}MANAGER_SYSTEM" . "|{$role_prefix}CANTEEN";
    }
    public static function ropManagerContent(): string
    {
        global $role_prefix;
        return 'role_or_permission:' . RoleType::SUPER_ADMIN->value . "|" . RoleType::LIBRARIAN->value . "|{$role_prefix}" . RoleType::ADMIN->value . "|{$role_prefix}MANAGER_CONTENT";
    }
    public static function ropContent(): string
    {
        global $role_prefix;
        return 'role_or_permission:' . RoleType::SUPER_ADMIN->value . "|" . RoleType::LIBRARIAN->value . "|{$role_prefix}" . RoleType::ADMIN->value . "|{$role_prefix}MANAGER_CONTENT" . "|{$role_prefix}AUTHOR";
    }
    ...
    public static function getPeriodBeginEnd($config): array
    {
        global $currentSystem, $period;
        $res = [
            'period_begin_at' => '',
            'period_end_at' => ''
        ];
        if (!empty($config['c_begin'])) {
            $res['period_begin_at'] = $config['c_begin'];
        }
        if (!empty($config['c_end'])) {
            $res['period_end_at'] = $config['c_end'];
        }
        if (!empty($currentSystem->settings)) {
            $periodArr = explode('-', $period);
            if (empty($res['period_begin_at'])) {
                if (!empty($currentSystem->settings->period_begin_at)) {
                    $res['period_begin_at'] = $currentSystem->settings->period_begin_at . '/' . $periodArr[0];
                } else {
                    $res['period_begin_at'] = '05/09/' . $periodArr[0];
                }
            }
            if (empty($res['period_end_at'])) {
                if (!empty($currentSystem->settings->period_end_at)) {
                    $res['period_end_at'] = $currentSystem->settings->period_end_at . '/' . $periodArr[1];
                } else {
                    $res['period_end_at'] = '31/05/' . $periodArr[1];
                }
            }
        } else {
            $periodArr = explode('-', $period);
    ...
    public static function isSupporter(): bool
    {
        ...
        if (!($currentUser instanceof User))
            return false;
        return $currentUser->hasRole([RoleType::LIBRARIAN->value, RoleType::SUPER_ADMIN->value]);
    }
    public static function isAdmin(): bool
    {
        global $role_prefix, $currentUser;
        if (!$currentUser)
            return false;
        if (!($currentUser instanceof User))
            return false;
        if (!$currentUser->hasRole(RoleType::ADMIN->value))
            return false;
        if (!$currentUser->arrRoles)
            return false;
        if (in_array($role_prefix . RoleType::ADMIN->value, $currentUser->arrRoles))
            return true;
        return false;
    }
