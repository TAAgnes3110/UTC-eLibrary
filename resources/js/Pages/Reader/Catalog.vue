...
onMounted(() => {
    try {
        sessionStorage.removeItem('reader_catalog_stats_bump_v1')
    } catch {
        /* ignore */
    }
    if (consumeReaderCatalogBooksStale()) {
...
/** Thể loại hiển thị — fallback khi prop cũ thiếu `category_label`. */
function catalogCategoryLabel(b) {
    const label = String(b?.category_label ?? '').trim()
    if (label) return label
    if (b?.resource_type === 'digital' || b?.is_digital === true) {
        return 'Đồ án, luận văn'
    }
    const alt = String(b?.classification_name || b?.resource_type_label || '').trim()
    return alt || '—'
}

function catalogViewCount(b) {
    const n = Number(b?.view_count ?? b?.digital_view_count ?? 0)
                    ...
                    >
                        ...
                        <div class="flex flex-1 flex-col p-4">
                            ...
                            <p class="mt-3 line-clamp-2 text-[11px] text-slate-500 dark:text-slate-400">
                                <span class="font-semibold text-slate-600 dark:text-slate-300">{{ C.categoryLabel }}:</span>
                                {{ catalogCategoryLabel(b) }}
                            </p>
                            <div class="mt-auto flex items-center justify-between border-t border-slate-100 pt-3 dark:border-slate-800">
                                ...
                                >
                                    ...
                                    {{ formatViewCount(catalogViewCount(b)) }} {{ C.viewsLabel }}
                                </span>
