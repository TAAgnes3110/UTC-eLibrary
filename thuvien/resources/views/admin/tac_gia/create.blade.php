@extends('layouts.admin')

@section('title', 'Thêm Tác giả mới')

@section('content')
<div class="container-fluid">
    <h1 class="mt-4">Thêm Tác giả mới</h1>
    <ol class="breadcrumb mb-4">
        <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Trang chủ</a></li>
        <li class="breadcrumb-item"><a href="{{ route('tac-gia.index') }}">Tác giả</a></li>
        <li class="breadcrumb-item active">Thêm mới</li>
    </ol>
    
    <div class="card mb-4">
        <div class="card-header">
            <i class="fas fa-user-plus mr-1"></i>
            Thông tin tác giả
        </div>
        <div class="card-body">
            <form action="{{ route('tac-gia.store') }}" method="POST">
                @csrf
                
                <div class="form-group mb-3">
                    <label for="ten_tac_gia">Tên tác giả <span class="text-danger">*</span></label>
                    <input type="text" class="form-control @error('ten_tac_gia') is-invalid @enderror" id="ten_tac_gia" name="ten_tac_gia" value="{{ old('ten_tac_gia') }}" required>
                    @error('ten_tac_gia')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                
                <div class="form-group mb-3">
                    <label for="ngay_sinh">Ngày sinh</label>
                    <input type="date" class="form-control @error('ngay_sinh') is-invalid @enderror" id="ngay_sinh" name="ngay_sinh" value="{{ old('ngay_sinh') }}">
                    @error('ngay_sinh')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                
                <div class="form-group mb-3">
                    <label for="tieu_su">Tiểu sử</label>
                    <textarea class="form-control @error('tieu_su') is-invalid @enderror" id="tieu_su" name="tieu_su" rows="5">{{ old('tieu_su') }}</textarea>
                    @error('tieu_su')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                
                <button type="submit" class="btn btn-primary">Thêm tác giả</button>
                <a href="{{ route('tac-gia.index') }}" class="btn btn-secondary">Hủy</a>
            </form>
        </div>
    </div>
</div>
@endsection